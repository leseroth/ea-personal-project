...
public class PromPanel extends FormCanvas {

    private TextFieldComponent numeroTelefonicoTF, claveVendedorTF;

    public PromPanel(CustomCanvas fl,String title){
        super(fl);
    ...
    public void commandAction(int command) {
        switch (command) {
            case LEFT_SOFTKEY:
                if (validData())
                { Control.control.smsmanager.sendTextmessage(Constants.NUMERO_CORTO_PROM, getPromMessage()); }
                break;
            case RIGHT_SOFTKEY:
                run = false;
    ...
    private String getPromMessage() {
        String res = "";
        res = Constants.LLAVE_ACCESO + Constants.TRAMA_PROM + CalendarData.getDate() + CalendarData.getHour();
