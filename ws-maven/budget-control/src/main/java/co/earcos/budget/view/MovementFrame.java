...
public class MovementFrame extends JFrame implements ActionListener {
	...
	private static final long serialVersionUID = -6045695349583561898L;
	// buttons
	...
	public MovementFrame(DayData day, DayPanel main) {
		...
		dayResumePanel.initDayResumePanel(
				ExpenseControlFrame.controlFrame.getMonthData(), dayData);
	...
	private JScrollPane initMovementListPanel() {
		...
		tableModel = new MovementTableDataModel(false,
				dayData.getMovementList());
	...
	private JPanel initMovementPanel() {
		...
		valueField = new JFormattedTextField(new DefaultFormatterFactory(
				new NumberFormatter(valueDisplayFormat), new NumberFormatter(
		...
		observationField.setDocument(new FieldValidator(100,
	...
	private void actionAddMovement() {
		if (isValidForm()) {
			...
			movementVO.setAccount(Account.values()[accountCombo
					.getSelectedIndex()]);
			...
			movementVO.setValue(adjustCheck.isSelected() ? value
					- dayResumePanel.getDayResumeData().getAccountTotal(
							movementVO.getAccount()) : value);
			...
			JOptionPane.showMessageDialog(this, "Falta el valor", "Error",
	...
	private void actionRemoveMovement() {
		if (table.getSelectedRows().length != 0) {
			for (int c : table.getSelectedRows()) {
				...
				dayData.getRemovedMovementList().add(
						dayData.getMovementList().get(index));
	...
	private void actionButtonSave() {
		...
		try {
			...
			dayData.getAddedMovementList().removeAll(
					dayData.getRemovedMovementList());
