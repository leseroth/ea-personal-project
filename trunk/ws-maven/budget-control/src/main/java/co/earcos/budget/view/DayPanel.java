...
public class DayPanel extends JPanel implements MouseListener {
    ...
    private JLabel initValueLabel(double value, Account account) {
        ...
        if (value == 0) {
            ...
            ImageIcon icon = createImageIcon(value > 0 ? Constants.INGRESO + account.getLabel() : Constants.EGRESO + account.getLabel());
    ...
    private ImageIcon createImageIcon(String path) {
        try {
            InputStream is = Util.getResource("images/" + path + Constants.ICON_FORMAT);
            Image image = ImageIO.read(is);
            return new ImageIcon(image);
        } catch (IOException ex) {
            log.warn("No se encuentra el archivo: " + path + Constants.ICON_FORMAT, ex);
            return null;
        }
    }
